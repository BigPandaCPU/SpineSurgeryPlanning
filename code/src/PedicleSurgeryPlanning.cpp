...
void PedicleSurgeryPlanning::CreateAndShowFinalActors()
{
	...
	all_actors.insert(all_actors.end(), left_bound_points_min_actors.begin(), left_bound_points_min_actors.end());
	all_actors.insert(all_actors.end(), right_bound_points_min_actors.begin(), right_bound_points_min_actors.end());
	ShowActors(all_actors, "show plan result");
}
...
void PedicleSurgeryPlanning::GetTheTrueFitPlaneNormal(const vector<float>& points, vector<float>& plane_center, vector<float>& plane_normal)
{
	...
	vector<float> reference_normal = NormalizeVector(m_top_points_center, plane_center);
