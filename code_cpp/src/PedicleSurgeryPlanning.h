...
using namespace std;
#define PI 3.1415926
#define TwoCentersDistanceThreshold 5.0  //截面中心点和特征点中心点之间的距离阈值，超过该阈值，则检测的截面无效
...
public:
	...
	static void FitPlaneFromPointsBySVD(vector<float>& fit_plane_center, vector<float>& fit_plane_normal, const vector<float>& points);

	void SaveSpineSurgeryPlanning2Png(const string& save_png_file);
	...
	static vector<float> GetPointsMean(const vector<float>& points);
	...
	static vector<float> GetProjectedPointOnPlane(const vector<float>& source_point, const vector<float>& plane_normal, const vector<float>& plane_center);
	...
	static vector<float> GetTheMeanOfTwoPoints(const vector<float>& pointsA, const vector<float>& pointsB);
	static vector<float> GetTheMinusOfTwoPoints(const vector<float>& pointsA, const vector<float>& pointsB, float factors = 1.0);
